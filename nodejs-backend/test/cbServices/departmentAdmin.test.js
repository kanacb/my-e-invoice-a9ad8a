...
const service = app.service("departmentAdmin").Model;
let patch = {};
let testData = [];
...
describe("departmentAdmin service", () => {
  ...
  it("create departmentAdmin data", async () => {
    ...
    // create a object array of departmentAdmin test schema model
    testData = [
      {
    ...
    if (!results || results.length === 0)
      assert.fail("departmentAdmin creation failed!");
    assert.ok(service, `Created (${results.length} departmentAdmin) success!`);
  });

  it("verify departmentAdmin creation", async () => {
  ...
  it("patch departmentAdmin", async () => {
    for (let i = 0; i < results.length; i++) {
      const patched = await service.findByIdAndUpdate(results[i]._id, patch, {
        new: true,
      });
      assert.ok(patched, `departmentAdmin ${patched} patched!`);
      assert.strictEqual(patched.type, patch.type);
    }
  });

  it("remove all departmentAdmin test data", async () => {
    for (let i = 0; i < results.length; i++) {
      const removed = await service.findByIdAndDelete(results[i]._id);
      assert.ok(removed, `departmentAdmin data ${results[i].number} removed!`);
    }
  });
