...
const service = app.service("measurements").Model;
const patch = {
  measurement: "measurement updated",
};
let testData = [];
...
describe("measurements service", () => {
  ...
  it("create measurements data", async () => {
    const standardUser = await usersService.findOne({
      email: "standard@example.com",
    });

    // create a object array of measurements test schema model
    testData = [
      {
        measurement: "Lbs",
    ...
    if (!results || results.length === 0)
      assert.fail("measurements creation failed!");
    assert.ok(service, `Created (${results.length} measurements) success!`);
  });

  it("verify measurements creation", async () => {
  ...
  it("patch measurements", async () => {
    for (let i = 0; i < results.length; i++) {
      const patched = await service.findByIdAndUpdate(results[i]._id, patch, {
        new: true,
      } );
      assert.ok(patched, `measurements ${patched} patched!`);
      assert.strictEqual(patched.measurement, patch.measurement);
    }
  });

  it("remove all measurements test data", async () => {
    for (let i = 0; i < results.length; i++) {
      const removed = await service.findByIdAndDelete(results[i]._id);
      assert.ok(removed, `measurements data ${results[i].number} removed!`);
    }
  });
