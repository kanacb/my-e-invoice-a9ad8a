...
const service = app.service("eInvoiceTypes").Model;
const patch = {
  eInvoiceTypes: "eInvoiceTypes updated",
};
let testData = [];
...
describe("eInvoiceTypes service", () => {
  ...
  it("create eInvoiceTypes data", async () => {
    const standardUser = await usersService.findOne({
      email: "standard@example.com",
    });

    // create a object array of eInvoiceTypes test schema model
    testData = [
      {
        eInvoiceTypes: "Admin",
    ...
    if (!results || results.length === 0)
      assert.fail("eInvoiceTypes creation failed!");
    assert.ok(service, `Created (${results.length} eInvoiceTypes) success!`);
  });

  it("verify eInvoiceTypes creation", async () => {
  ...
  it("patch eInvoiceTypes", async () => {
    for (let i = 0; i < results.length; i++) {
      const patched = await service.findByIdAndUpdate(results[i]._id, patch, {
        new: true,
      } );
      assert.ok(patched, `eInvoiceTypes ${patched} patched!`);
      assert.strictEqual(patched.eInvoiceTypes, patch.eInvoiceTypes);
    }
  });

  it("remove all eInvoiceTypes test data", async () => {
    for (let i = 0; i < results.length; i++) {
      const removed = await service.findByIdAndDelete(results[i]._id);
      assert.ok(removed, `eInvoiceTypes data ${results[i].number} removed!`);
    }
  });
