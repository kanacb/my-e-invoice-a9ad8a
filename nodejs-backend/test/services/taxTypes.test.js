...
const service = app.service("taxTypes").Model;
const patch = {
...
describe("taxTypes service", () => {
  ...
  it("create taxTypes data", async () => {
    const standardUser = await usersService.findOne({
      email: "standard@example.com",
    });

    // create a object array of taxTypes test schema model
    testData = [
      {
        taxType: "VAT",
    ...
    if (!results || results.length === 0)
      assert.fail("taxTypes creation failed!");
    assert.ok(service, `Created (${results.length} taxTypes) success!`);
  });

  it("verify taxTypes creation", async () => {
  ...
  it("remove all taxTypes test data", async () => {
    for (let i = 0; i < results.length; i++) {
      const removed = await service.findByIdAndDelete(results[i]._id);
      assert.ok(removed, `taxTypes data ${results[i].number} removed!`);
    }
  });
