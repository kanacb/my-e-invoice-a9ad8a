...
const service = app.service("departmentHOD").Model;
let patch = {};
let testData = [];
...
describe("departmentHOD service", () => {
  ...
  it("create departmentHOD data", async () => {
    ...
    // create a object array of departmentHOD test schema model
    testData = [
      {
    ...
    if (!results || results.length === 0)
      assert.fail("departmentHOD creation failed!");
    assert.ok(service, `Created (${results.length} departmentHOD) success!`);
  });

  it("verify departmentHOD creation", async () => {
  ...
  it("patch departmentHOD", async () => {
    for (let i = 0; i < results.length; i++) {
      const patched = await service.findByIdAndUpdate(results[i]._id, patch, {
        new: true,
      });
      assert.ok(patched, `departmentHOD ${patched} patched!`);
      assert.strictEqual(patched.type, patch.type);
    }
  });

  it("remove all departmentHOD test data", async () => {
    for (let i = 0; i < results.length; i++) {
      const removed = await service.findByIdAndDelete(results[i]._id);
      assert.ok(removed, `departmentHOD data ${results[i].number} removed!`);
    }
  });
