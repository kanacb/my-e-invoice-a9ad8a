...
const service = app.service("classificationCodes").Model;
const patch = {
  description: "classificationCodes updated",
};
let testData = [];
...
describe("classificationCodes service", () => {
  ...
  it("create classificationCodes data", async () => {
    const standardUser = await usersService.findOne({
      email: "standard@example.com",
    });

    // create a object array of classificationCodes test schema model
    testData = [
      {
        classificationCode: "Admin",
    ...
    if (!results || results.length === 0)
      assert.fail("classificationCodes creation failed!");
    assert.ok(service, `Created (${results.length} classificationCodes) success!`);
  });

  it("verify classificationCodes creation", async () => {
  ...
  it("patch classificationCodes", async () => {
    for (let i = 0; i < results.length; i++) {
      const patched = await service.findByIdAndUpdate(results[i]._id, patch, {
        new: true,
      } );
      assert.ok(patched, `classificationCodes ${patched} patched!`);
      assert.strictEqual(patched.type, patch.type);
    }
  });

  it("remove all classificationCodes test data", async () => {
    for (let i = 0; i < results.length; i++) {
      const removed = await service.findByIdAndDelete(results[i]._id);
      assert.ok(removed, `classificationCodes data ${results[i].number} removed!`);
    }
  });
