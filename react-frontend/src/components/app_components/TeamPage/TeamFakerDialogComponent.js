...
import teamFakerFactory from "./teamFakerFactory";
...
const TeamFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = teamFakerFactory(props.user,fakerCount,usersIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("team").create(elem));
...
export default connect(mapState, mapDispatch)(TeamFakerDialogComponent);
