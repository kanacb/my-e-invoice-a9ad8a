...
const TeamCreateDialogComponent = (props) => {
  ...
  const onSave = async () => {
    ...
    setLoading(true);
    try {
      await client.service("team").patch(_entity._id, _data);
      const eagerResult = await client.service("team").find({
        query: {
          $limit: 10000,
          _id: { $in: [_entity._id] },
          $populate: [
      ...
      props.alert({
        type: "success",
        title: "Edit info",
        message: "Info team updated successfully",
      });
  ...
  return (
    ...
    >
      <div
        ...
        style={{ maxWidth: "55vw" }}
        role="team-edit-dialog-component"
      >
