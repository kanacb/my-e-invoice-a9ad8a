...
const TeamDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
<Column field="users" header="Users" body={dropdownTemplate1} filter={selectedFilterFields.includes("users")} hidden={selectedHideFields?.includes("users")}  style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Team Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="team"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Team" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
