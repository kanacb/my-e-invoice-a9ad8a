...
const BuyersFakerDialogComponent = (props) => {
  ...
  const [buyersAddressCountryNameIds, setbuyersAddressCountryNameIds] =
    useState();
  const [buyersAddressStateNameIds, setbuyersAddressStateNameIds] = useState();
  const [theFirstBuyersContactNumberIds, settheFirstBuyersContactNumberIds] =
    useState();
  ...
  const onRun = async () => {
    let fakeData = buyersFakerFactory(
      props.user,
      fakerCount,
      buyersNameIds,
      identifierTypeIds,
    ...
    const promises = fakeData.map((elem) =>
      client.service("buyers").create(elem),
    );
